...
static void game_spawn_explosion(int x, int y) {
	for (int ex = 0; ex < MAX_EXPLOSIONS; ex++) {
		if (g_explosions[ex].state == EXPLOSION_STATE_INACTIVE) {
			g_explosions[ex].state = EXPLOSION_STATE_EXPANDING;
			g_explosions[ex].x = x;
			g_explosions[ex].y = y;
...
static bool process_bullet_collision(int i) {
	if (g_bullets[i].is_enemy) {
		if (game_get_player_blink() == 0 && check_player_pixel_collision(g_bullets[i].x, g_bullets[i].y)) {
			g_bullets[i].state = BULLET_STATE_INACTIVE;
		...
		for (int e = 0; e < MAX_ENEMIES; e++) {
			if (enemies[e].state == ENEMY_STATE_INACTIVE) continue;
			...
			if (game_check_collision(g_bullets[i].x, g_bullets[i].y, 1, 4, enemies[e].x, enemies[e].y, ew, eh)) {
				...
				task_post_common_msg(AC_TASK_GAME_ENEMY_ID, AC_GAME_ENEMY_HIT, (uint8_t*)&hit_msg, sizeof(hit_msg));
...
static void update_bullet_movement(int i) {
	...
	if (g_bullets[i].is_enemy) {
		...
		if (move_enemy_bullet) {
			g_bullets[i].y += drop;
			if (g_bullets[i].y > 64) g_bullets[i].state = BULLET_STATE_INACTIVE;
		}
	} else {
		int speed = (game_get_tick_count() % 5 < 2) ? 2 : 1;
		g_bullets[i].y -= speed;
		if (g_bullets[i].y < 0) g_bullets[i].state = BULLET_STATE_INACTIVE;
	}
}
...
static void game_bullets_update() {
	for (int i = 0; i < MAX_BULLETS; i++) {
		if (g_bullets[i].state == BULLET_STATE_INACTIVE) continue;
	...
	if (game_get_tick_count() % 2 == 0) {
		for (int ex = 0; ex < MAX_EXPLOSIONS; ex++) {
			if (g_explosions[ex].state != EXPLOSION_STATE_INACTIVE) {
				...
				if (g_explosions[ex].timer <= 0) g_explosions[ex].state = EXPLOSION_STATE_INACTIVE;
			}
		}
	}
}
...
void game_bullet_task(ak_msg_t* msg) {
	switch (msg->sig) {
		case AC_GAME_START_REQ:
		{
			...
			for (int i = 0; i < MAX_BULLETS; i++) g_bullets[i].state = BULLET_STATE_INACTIVE;
			for (int i = 0; i < MAX_EXPLOSIONS; i++) g_explosions[i].state = EXPLOSION_STATE_INACTIVE;
		}
		break;
		...
		case AC_GAME_SPAWN_BULLET: 
		{
			...
			for (int i = 0; i < MAX_BULLETS; i++) {
				if (g_bullets[i].state == BULLET_STATE_INACTIVE) {
					g_bullets[i].state = BULLET_STATE_FIRED;
